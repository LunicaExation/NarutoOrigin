...
public class CommandExecuteTaijutsu extends CommandBase {

    @Override
    public String getName() {
        return "executeTaijutsu";
    }

    @Override
    public String getUsage(ICommandSender sender) {
        return "/executeTaijutsu <playerName>";
    }

    @Override
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws WrongUsageException {
        if (args.length < 1) {
            throw new WrongUsageException("Usage: /executeTaijutsu <playerName>");
        }

        String playerName = args[0];

        server.getCommandManager().executeCommand(server, "advancement grant " + playerName + " from narutomod:openedgates");
    }
